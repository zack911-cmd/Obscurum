...
import { BookOpen, Target, AlertTriangle, Copy, Users, Server, Key, Terminal, CheckCircle2, XCircle, ChevronRight, RotateCcw } from 'lucide-react'
  ...
  return (text: string, key: string) => {
    const fallback = () => {
      try {
        ...
        // Clipboard not available — silently fail, but the button shows "Copy"
        // not "Copied" so user can still select manually
...
const CopyBtn = memo(function CopyBtn({
  ...
  return (
    <button
      ...
      className="ml-2 text-xs px-2 py-1 hover:bg-white/10 rounded flex-shrink-0 flex items-center gap-1 text-ghost-text-dim transition-colors"
...
export default function BloodHoundCoach() {
  ...
  // Stable copy handler that doesn't recreate on each render
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {/* Warning */}
      <div className="ghost-card border border-purple-500/30 bg-purple-950/20 p-4 rounded-2xl flex gap-3 mb-4">
        <AlertTriangle className="text-purple-400 mt-0.5 flex-shrink-0" />
      ...
      {/* Hands-on nudge */}
      <div className="ghost-card border border-amber-500/30 bg-amber-950/10 p-4 rounded-2xl flex gap-3 mb-6">
        <Terminal className="text-amber-400 mt-0.5 flex-shrink-0" />
        <div className="text-sm text-amber-200">
          Reading this without a live domain in front of you is memorization, not skill.
          Go to <strong>Lab + Collection</strong> and stand up GOAD before going deeper than the Overview tab.
        </div>
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          <div className="ghost-card p-6 rounded-2xl">
            <h2 className="text-xl font-semibold mb-3">What is BloodHound?</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="ghost-card p-6 rounded-2xl">
              <h3 className="font-semibold text-purple-400 mb-3">Why It's Powerful</h3>
              <ul className="text-sm space-y-2 text-ghost-text-dim list-disc pl-5">
            ...
            <div className="ghost-card p-6 rounded-2xl">
              <h3 className="font-semibold text-purple-400 mb-3">Key Components</h3>
              <ul className="text-sm space-y-2 text-ghost-text-dim list-disc pl-5">
                <li><strong>SharpHound</strong> — C# data collector, runs on a domain-joined host or with valid creds, queries AD via LDAP + Windows APIs</li>
                <li><strong>BloodHound GUI / CE</strong> — Neo4j-backed graph visualization + query interface</li>
                <li><strong>Cypher</strong> — Neo4j's query language, structurally similar to SQL but pattern-matches graph paths instead of table rows</li>
              </ul>
            </div>
          </div>

          <div className="ghost-card p-6 rounded-2xl border border-purple-500/20">
            <h3 className="font-semibold text-purple-400 mb-2">Prerequisite check</h3>
      ...
      {/* Core Concepts */}
      {activeTab === 'concepts' && (
        <div className="ghost-card p-6 rounded-2xl space-y-6">
          <h2 className="text-xl font-semibold">Core Concepts</h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-4 bg-ghost-bg-soft rounded-xl">
              <h3 className="font-semibold text-purple-400 mb-2">Nodes (Objects)</h3>
              <ul className="text-sm text-ghost-text-dim space-y-1.5">
                <li><strong>User</strong> — domain account; carries flags like hasspn, dontreqpreauth</li>
                <li><strong>Computer</strong> — machine account; also has sessions, local admin rights, delegation flags</li>
                <li><strong>Group</strong> — security group; membership is transitive (nested groups matter)</li>
                <li><strong>Domain</strong> — the AD domain; DCSync rights on this node are the endgame</li>
                <li><strong>OU</strong> — organizational unit; GPOs apply here, not permissions directly</li>
                <li><strong>GPO</strong> — Group Policy Object; if you can edit one linked to a privileged OU, you own everything under it</li>
              </ul>
            </div>

            <div className="p-4 bg-ghost-bg-soft rounded-xl">
              <h3 className="font-semibold text-purple-400 mb-2">Edges (Relationships) — with mechanism</h3>
              <ul className="text-sm text-ghost-text-dim space-y-2">
                <li><strong>MemberOf</strong> — group membership, transitive through nesting. If A is MemberOf B and B is MemberOf Domain Admins, A effectively is a Domain Admin, even if no one intended that.</li>
                <li><strong>AdminTo</strong> — local admin on a computer, via the local Administrators group. Local admin means you can dump LSASS, meaning any cached creds or Kerberos tickets on that box are yours.</li>
                <li><strong>HasSession</strong> — a user is currently logged on. Matters because their credential material (password hash, TGT) is sitting in that machine's memory while they're logged in.</li>
                <li><strong>GenericAll</strong> — full control over the target object's attributes. On a user, this lets you reset their password directly (no crack needed). On a computer, combined with Resource-Based Constrained Delegation, it lets you impersonate any user against that computer.</li>
                <li><strong>GenericWrite</strong> — write access to most attributes. On a user, you can write a fake SPN and then Kerberoast them, or push RBCD onto a computer object.</li>
                <li><strong>Owns</strong> — object ownership implies you can rewrite its ACL entirely, including granting yourself GenericAll.</li>
                <li><strong>CanRDP / CanPSRemote</strong> — remote access rights — these are how you actually get a session on a box once you have credentials, not a privilege escalation edge by themselves.</li>
              </ul>
            </div>
          </div>

          <div className="p-4 bg-ghost-bg-soft rounded-xl border border-purple-500/20">
            <h3 className="font-semibold text-purple-400 mb-2">Why "the edge exists" isn't enough</h3>
            <p className="text-sm text-ghost-text-dim">
      ...
      {activeTab === 'attackpaths' && (
        <div className="ghost-card p-6 rounded-2xl space-y-6">
          <h2 className="text-xl font-semibold">Attack Chains (not isolated techniques)</h2>
          <p className="text-sm text-ghost-text-dim">
          ...
          <div className="space-y-5">
            {[
              ...
              <div key={index} className="p-4 bg-ghost-bg-soft rounded-xl">
                <h3 className="font-semibold mb-2 flex items-center gap-2">
                  <ChevronRight size={16} className="text-purple-400" /> {item.title}
                </h3>
                <ol className="text-sm text-ghost-text-dim space-y-1 pl-5 list-decimal mb-3">
                  {item.steps.map((s, i) => <li key={i}>{s}</li>)}
                </ol>
                <p className="text-xs text-purple-300/80 border-t border-ghost-border pt-2">
      ...
      {activeTab === 'queries' && (
        <div className="ghost-card p-6 rounded-2xl">
          <h2 className="text-xl font-semibold mb-2">Cypher Queries</h2>
          ...
          <div className="space-y-4 text-sm">
            {[
              ...
              <div key={i} className="bg-ghost-bg-soft p-4 rounded-xl">
                <div className="font-semibold mb-1">{q.name}</div>
                <div className="flex justify-between items-center bg-black/40 p-2 rounded font-mono text-xs">
      ...
      {activeTab === 'collection' && (
        <div className="ghost-card p-6 rounded-2xl space-y-6">
          <h2 className="text-xl font-semibold">Lab Setup + Data Collection</h2>

          <div className="p-4 bg-ghost-bg-soft rounded-xl border border-amber-500/20">
            <h3 className="font-semibold mb-2 text-amber-300">Set this up before anything else</h3>
            <p className="text-sm text-ghost-text-dim mb-2">
              <strong>GOAD (Game of Active Directory)</strong> deploys a small multi-domain, multi-forest AD lab
            ...
            <div className="bg-black/40 p-3 rounded font-mono text-xs flex justify-between items-center">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-ghost-bg-soft rounded-xl">
              <h3 className="font-semibold mb-2">Basic Collection (All methods)</h3>
              <div className="bg-black/40 p-3 rounded font-mono text-sm flex justify-between items-center">
              ...
              <p className="text-xs text-ghost-text-dim mt-2">
                <code>-c All</code> runs every collection method: group membership, sessions, local admin,
            ...
            <div className="p-4 bg-ghost-bg-soft rounded-xl">
              <h3 className="font-semibold mb-2">Reduced / quieter collection (real syntax)</h3>
              <div className="bg-black/40 p-3 rounded font-mono text-sm flex justify-between items-center">
              ...
              <p className="text-xs text-ghost-text-dim mt-2">
                <code>DCOnly</code> pulls everything queryable from the Domain Controller via LDAP only — no
                ...
                "stealth" — <code>DCOnly</code> is the actual reduced-footprint method professionals use, since
                it skips the noisy host-by-host SMB/RPC calls that trigger EDR and generate massive event volume.
              </p>
            </div>

            <div className="p-4 bg-ghost-bg-soft rounded-xl">
              <h3 className="font-semibold mb-2">Combining specific methods</h3>
              <div className="bg-black/40 p-3 rounded font-mono text-sm flex justify-between items-center">
              ...
              <p className="text-xs text-ghost-text-dim mt-2">Comma-separated collection methods let you scope exactly what you generate traffic for.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'defense' && (
        <div className="ghost-card p-6 rounded-2xl space-y-6">
          <h2 className="text-xl font-semibold text-purple-400">Detection & Defense</h2>

          <div className="space-y-4 text-sm">
            <div className="p-4 bg-ghost-bg-soft rounded-xl">
              <h3 className="font-semibold mb-2">How Defenders Detect Collection</h3>
              <ul className="list-disc pl-5 text-ghost-text-dim space-y-1">
                ...
                <li><code>-c All</code> generates SMB session enumeration (NetSessionEnum) and remote registry calls against many hosts in sequence — a very identifiable fan-out pattern</li>
            ...
            <div className="p-4 bg-ghost-bg-soft rounded-xl">
              <h3 className="font-semibold mb-2">How to Defend (mechanism-level, matched to the attack chains above)</h3>
              <ul className="list-disc pl-5 text-ghost-text-dim space-y-1">
                <li><strong>Against Kerberoasting/AS-REP roasting:</strong> enforce long, random passwords on service accounts (25+ chars via gMSA where possible removes the crackable-hash problem entirely), and require preauth on all accounts</li>
                <li><strong>Against ACL abuse chains:</strong> run BloodHound against your own domain regularly and treat any unexpected GenericAll/WriteDacl path into a privileged group as a finding, not noise</li>
                <li><strong>Against DCSync/Golden Ticket:</strong> tightly restrict Replicating Directory Changes rights, and monitor for DsGetNCChanges calls from non-DC sources (Event ID 4662 with the right GUIDs)</li>
                <li><strong>Structural fix, not detection:</strong> Tiered Administration model — Tier 0 (DC/DA) credentials never touch Tier 1/2 machines, which removes most of the HasSession-based credential theft paths entirely</li>
      ...
      {/* Quiz — always mounted, hidden when not active so state persists */}
  ...
  return (
    <div className="ghost-card p-6 rounded-2xl space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold">Self-Check — Active Recall</h2>
      ...
      <div className="p-5 bg-ghost-bg-soft rounded-xl">
        <div className="text-xs text-purple-400 font-mono mb-2">QUESTION {idx + 1} / {questions.length}</div>
        <p className="font-medium mb-4">{questions[idx].q}</p>
        ...
        {!revealed ? (
          ...
          <div className="space-y-4">
            <div className="p-4 bg-black/30 rounded-lg text-sm text-ghost-text-dim border border-purple-500/20">
              {questions[idx].a}
            </div>
              ...
              <button
                ...
                className={`text-sm px-4 py-2 rounded-lg flex items-center gap-2 transition-colors ${
                  results[idx] === true
                    ...
                    : 'bg-white/5 text-ghost-text-dim hover:bg-white/10'
                }`}
              >
      ...
      {answered === questions.length && (
        <div className="p-4 bg-ghost-bg-soft rounded-xl flex flex-wrap justify-between items-center gap-3">
          ...
          <button
            onClick={reset}
            className="text-xs px-3 py-1.5 rounded-lg border border-ghost-border flex items-center gap-1.5 flex-shrink-0 hover:bg-white/5 transition-colors"
          >
